...
while true; do
	...
	if (( date_count==60 )); then
		"/home/alzhar/.scripts/dwm/update_date.sh"
		date_count=1
	else
		((date_count++))
	fi
	...
	if (( battery_count==180 )); then
		"/home/alzhar/.scripts/dwm/update_battery.sh"
		battery_count=1
	else
		((battery_count++))
	fi
	...
	if (( volume_count==36000 )); then
		"/home/alzhar/.scripts/dwm/update_volume.sh"
		volume_count=1
	else
		((volume_count++))
	fi
	...
	if (( internet_status_count==5 )); then
		"/home/alzhar/.scripts/dwm/update_internet_status.sh"
	...
	if (( weather_count==1800 )); then
		"/home/alzhar/.scripts/dwm/update_weather.sh"
		weather_count=1
	else
		((weather_count++))
	fi
	...
	if (( disk_use_count==180 )); then
		"/home/alzhar/.scripts/dwm/update_disk_use.sh"
		disk_use_count=1
	else
		((disk_use_count++))
	fi
